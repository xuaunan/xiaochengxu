...
本文记录本轮围绕管理端数据大盘、用户端订单流程、司机端轨迹模式和后台同步链路做过的改动，方便后续验证、验收和二次维护。
...
- 规划速度按每 1.5 公里约 1 分钟推进；遇到随机红灯时车辆暂停、时间继续增加，绿灯后继续按路线匀速行驶。
- 地图小车图标代表当前虚拟车辆位置，车头方向跟随当前道路段方向变化。
- 乘客端、司机端和管理后台看到的是同一条规划轨迹。
- 不使用司机手机真实定位，不暴露真实位置。
- 本次修复后，司机端不再每次用“当前位置 -> 目标点”重新规划短路线；每个订单每个阶段固定使用同一条完整路线：接驾阶段为“随机司机起点 -> 上车点”，行程阶段为“上车点 -> 终点”。
- 司机端行程页和接单大厅都会每 3 秒推进并上报一次规划车辆位置；即使司机停留在接单大厅，也会继续同步小车位置、已行驶距离、耗时和进度。
- 速度上报允许 `0km/h` 作为红灯等待的合法值；绿灯通行时按规划规则恢复为 `90km/h`，避免绿灯状态仍显示 `0km/h`。
...
规划轨迹的 `DEMO_ROUTE` 备注会附带摘要字段，例如已用秒数、已行驶距离、剩余距离、路线百分比和等待秒数。后端运行态会优先读取这些摘要，避免后台用直线距离粗算。
...
- 后台轨迹记录表不再展示具体经纬度；只保留上报时间、角色、行驶进度、已行驶、行驶时长和等待/路况信息，方便验证时看同步进度但不暴露具体位置。
...
4. 用户端创建即时打车订单。
5. 司机端接单。
6. 用户端进入“司机已接单”页，看到司机从上车点附近规划接驾。
7. 管理后台进入订单详情，查看轨迹模式和轨迹记录。
8. 如需验证真实定位，再回司机端设置切换为“真实”，重新接单测试。
...
说明：后端 `package` 如果提示无法重命名 `target/sunshine-travel-1.0.0.jar`，通常是服务进程正在运行并占用 jar 文件，先停止服务后再打包即可。
